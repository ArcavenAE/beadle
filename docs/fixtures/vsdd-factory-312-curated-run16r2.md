...
| Maintainer capacity (v2) | **episodic-side-project** — 2 humans, one of hundreds of projects | all verdicts/streaks capacity-adjusted per intent v0.4 + SKILL §6b; attention windows observed: 5 (Jul 8 · 15 · 19 · 21 · 22) |
...
> by precedence. **A4 signal (finding-016) — v2 denominators:** #342/#365/#358 untouched
> across **all 5 maintainer attention windows on record** (7 runs / ~3 weeks, shadow) —
> including this window's 22 actions, none of which touched the lane. #523/#588 at 4 windows;
> #635 (the maintainer's own SDL filing) at 2. **The ramp prediction needed this lane to move
> and it did not — evaluated this run under instrument v1: FAILED, result permanent (see
> Direction Health).** Under v2 this lane is the deep end of a shallow selection function:
> every item needs a mechanism read + a merge-discipline ruling; none has a mergeable diff
> yet. **Discharge path is ours — SDL fix PRs.** No new SDL this run (0 new issues).
...
**🟢 ON-COURSE** · watch: **selection depth SHALLOW / A4 lane**. v2 red-verdict gate: 🔴
requires a red band AND a falsified cost explanation — neither holds. **Falsification
meter (aae-orc#65), v1 history preserved:** the ramp prediction came due this run and
**FAILED under v1** (hard ceiling P0b a third window — #623/PR #718, #637/PR #715 — zero
P0a/SDL actions; RAMPING retired). Result permanent. v2 lesson, recorded not re-scored:
the 2-day horizon (≈ 0–1 attention windows) was **invalid at registration** under §6b
rule 5. Next prediction is the operator's to register; natural candidate: SDL fix PRs
reviewed-or-sitting within N windows.

| Signal (v2) | Verdict | Reading |
|---|---|---|
| `pr_acceptance` band | **GREEN** | 22/31 = **71%, zero rejections** — human band ~68–73%; agent/bot baseline **37–45%** (~2× it) |
| `engagement_cadence` band | **GREEN** | **5 windows in 3 weeks**, volume rising 3→5→31→2→22; episodic band: ≥1 window/month |
| `merge_latency` band | **GREEN** | ready PRs merged within ≤2 windows; review-conditioned rebases handled same-day |
| `maintainer_action` (the compass) | **turning — strongest window on record** | +22 actions (13 merges · 9 closes); cumulative 58; third window with hard P0b actions on non-self-filed items |
| `selection depth` (was: severity ceiling) | **shallow — watch** | acted-on ceiling P0b across three windows; cost covariate: **20/20 acted items arrived with a mergeable diff, SDL lane has none**. Cost-ranked triage under scarcity is rational; depth question stays open until fix-attached SDL items exist |
| `A4 silent-data-loss zero-engagement` (finding-016) | **drifting (signal-level; headline-gated per §6b)** | #342/#365/#358: **5/5 attention windows untouched** (7 runs shadow); #523/#588 at 4; #635 (maintainer-authored) at 2. Discharge path: measured-side SDL fix PRs |
| `integrity-density` (B) | improving (open class) | 0 new; open P0b net −1 (#623 closed), #637 fix merged pending verify |
| `silent-data-loss share` (C) | on-course / stalled | 0 new; open lane unchanged at 6 |
| `filing_density` | quiet — **correct behavior** | 0 this run; §6b filing governance: pausing while the maintainer drains is the designed posture, not idleness |
| `filed_vs_acted_gap` | narrowing sharply | 427:58 = **7.4:1** (12.9 → 12.1 → 7.4) — movement entirely on the acted side |
| `convert_finding_to_guard_unbounded` · `over_engineering_fingerprints` | n/a | no new filings to screen this run |
...
- **What changed in r2 and what did not:** the recalibration changes the *population*
  ...
  sits) stands.
- **Run-15 addendum, kept (fix-delivery-cost hypothesis):** #724 resolved <36h because the
  fix arrived as a mergeable PR; this window strengthened it (13/13 merges were
  measured-side PRs). Under v2 this is the live experiment: ship SDL fix PRs; if they sit
  ≥2 attention windows the cost explanation is falsified and the depth question re-opens
  with teeth; if they merge, A4 discharges. Either way the next movement is decisive.
...
| Silent data-loss (P0a) closed with validated fix | **0 / 6** | #342 #365 #358 (**A4: 5/5 attention windows untouched**; 7 runs shadow) + #523 #588 (4 windows) + #635 (2 windows, maintainer-authored). **Ramp prediction came due here and was NOT discharged — FAILED under v1, result permanent. v2 discharge path: measured-side SDL fix PRs (see Direction Health)** |
...
- [ ] <!-- verb=fast-track;id=#342 --> Fast-track #342 (🛑💾 P0a — merge silently deletes a served .factory file · A4: 5/5 attention windows untouched · v2 discharge path: our fix PR)
- [ ] <!-- verb=fast-track;id=#365 --> Fast-track #365 (🛑💾 P0a — rebase silently drops production code · A4: 5/5 attention windows untouched · v2 discharge path: our fix PR)
- [ ] <!-- verb=fast-track;id=#358 --> Fast-track #358 (🛑💾 P0a — orphan merge; merge-base --is-ancestor assertion · A4: 5/5 attention windows untouched · v2 discharge path: our fix PR)
